...
void Syn5(int argc, char *argv[]){
	int N;
	float avg;

	printf("Πόσους αριθμούς έχετε δώσει; ");
	scanf("%d",&N);
	
	avg = mesos_oros(argc, argv, N);
	printf("Ο μέσος όρος %d αριθμου/ών που δώσατε από κονσόλα είναι: %.2f\n",N,avg);
	system ("pause");
}
...
double mesos_oros(int argc,char *argv[],int N){
	int i;
	float avg,num;
	long double sum = 0;
